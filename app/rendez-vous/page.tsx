...
import { User, Phone, Mail, Stethoscope, CalendarDays, Clock, PenLine } from 'lucide-react';
...
export default function RendezVousPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!fullName.trim() || !phone.trim() || !consent) {
      setError('Merci de remplir les champs requis et d’accepter les conditions.');
      return;
    }
  ...
  return (
    // NOTE: `scrolled` on <Navbar /> switches between the transparent hero
    // variant (light text, meant to sit over the dark hero image) and the
    // solid variant (white bg, dark text). This page has no hero behind the
    // navbar, so it always needs the SOLID state — hence `scrolled` is
    // hardcoded true here, not tied to an actual scroll position.
    // If Navbar.tsx uses a different prop/condition to pick that state,
    // swap this line to match it instead of editing Navbar itself.
    <div className="flex min-h-screen w-full flex-col items-center bg-white">
      <Navbar scrolled={true} />

      <main className="flex w-full flex-1 justify-center px-6 pb-24 pt-32 sm:pt-36">
        ...
        {submitted ? (
          <div className="rounded-xl border border-zinc-100 bg-zinc-50 px-6 py-10 text-center">
            <p className="text-lg font-medium text-zinc-900">Merci{fullName ? `, ${fullName.split(' ')[0]}` : ''} !</p>
            <p className="mt-2 text-sm text-zinc-500">
              Votre demande de rendez-vous a bien été envoyée. Nous vous contactons très vite.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <FormField icon={<Stethoscope className="h-5 w-5 text-zinc-400" />}>
              <select
                value={service}
            ...
            <div className="flex gap-4">
              <FormField icon={<CalendarDays className="h-5 w-5 text-zinc-400" />}>
                <input
                  type="date"
                  value={date}
            ...
            <label className="flex items-start gap-2.5 pt-2 text-sm text-zinc-500">
              ...
              <span>
                ...
                <a href="/confidentialite" className="underline underline-offset-2 hover:text-zinc-700">
